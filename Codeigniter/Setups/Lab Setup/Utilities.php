...
class Utilities extends CI_Model
{
    ...
    function insert($tableName, $post)
    {
        ...
        $this->db->insert($tableName, $post);
        $primaryKeyVal=$this->db->query("SELECT cols.table_name,
         cols.column_name,
         cols.position,
         cons.status,
         cons.owner
        ...
        ORDER BY cols.table_name, cols.position")->row();
        //print_r($primaryKeyVal);
        if(!empty($primaryKeyVal))
        {
          $colName=$primaryKeyVal->COLUMN_NAME;
          $maxidArray=$this->db->query("SELECT MAX($colName) MAX_ID FROM $tableName")->row();
          if(!empty($maxidArray))
          {
            $returnVar= $maxidArray->MAX_ID;
          }
          else {
            $returnVar=true;
            //return true;
        ...
        return $returnVar;
        //print_r($primaryKeyVal);
        //return $primaryKeyVal->COLUMN_NAME;
    ...
    {
        $mainTableName=strtoupper($mainTableName);
        $joinTableName=strtoupper($joinTableName);
    ...
    function get_addmission_roll_number($year,$session,$faculty,$department,$program)
    {
        ...
        //$row = $this->db->query("SELECT (IFNULL(MAX(SUBSTR(ADM_ROLL_NO,10)), 0)+1) next_sn
        //FROM applicant_personal_info WHERE ADM_ROLL_NO LIKE '$condition%'")->row();
        $row=$this->db->query(" SELECT NVL(MAX(SUBSTR(ADM_ROLL_NO,10)+1),0) NEXT_SN  FROM applicant_personal_info
 WHERE ADM_ROLL_NO LIKE '$condition'")->row();
        $sl = str_pad($row->NEXT_SN, 3, "0", STR_PAD_LEFT);
        return $condition . $sl;
    }
    function get_registration_no($year,$session,$faculty,$program)
    {
        ...
        $row = $this->db->query("SELECT (NVL(MAX(SUBSTR(REGISTRATION_NO,13)), 0)+1) next_sn FROM student_personal_info WHERE REGISTRATION_NO LIKE '$condition%'")->row();
    ...
    function batchProgList()
    {
        $query = $this->db->query("SELECT a.*,
                                           b.BATCH_TITLE,
                                           c.PROGRAM_NAME,
                                          (e.SESSION_NAME ||'-' || d.DINYEAR) AS SESSION_NAME
   ...
   function admissionSessionList(){
       ...
       return  $this->db->query("  SELECT a.*, b.SESSION_NAME  ||' - ' || a.DINYEAR AS SESSION_NAME
    FROM adm_ysession a LEFT JOIN ins_session b ON a.SESSION_ID = b.SESSION_ID
ORDER BY a.UD_SLNO DESC")->result();
   }
   function admissionSessionById($id){

       return  $this->db->query("SELECT a.*,  a.*, b.SESSION_NAME  ||' - ' || a.DINYEAR AS SESSION_NAME
                                  FROM adm_ysession a LEFT JOIN ins_session b ON a.SESSION_ID = b.SESSION_ID
                                 WHERE a.YSESSION_ID =$id")->row();
   }
   function academicSessionList(){

       return  $this->db->query("select a.*,(b.SESSION_NAME ||'-' || a.DINYEAR) AS SESSION_NAME from ins_ysession a
        left join ins_session b on a.SESSION_ID = b.SESSION_ID ORDER BY a.UD_SLNO DESC")->result();
   }

   function academicSessionById($id){

       return  $this->db->query("SELECT a.*, b.SESSION_NAME || ' - '|| a.DINYEAR  SESSION_NAME
   ...
   function getAllUserInfo()
   {
       return  $this->db->query("SELECT su.*,sug.USERGRP_NAME, sul.UGLEVE_NAME, a.*,b.DEPT_NAME,c.DESIGNATION
                                            ...
                                            LEFT JOIN hr_emp a on TO_CHAR(su.EMP_ID) =TO_CHAR(a.EMP_ID)
                                            LEFT JOIN ins_dept b on To_CHAR(su.DEPT_ID) = b.DEPT_ID
  ...
  function set_barcode($code)
  {
    ...
    function findAllByAttributeAsArr($tableName, $attribute)
    {
        return $this->db->get_where($tableName, $attribute)->result_array();
    }
    ...
    function findAttenStdData($EXP_ID, $SDL_DT, $YSESSION_ID, $DEGREE_ID, $FACULTY_ID, $DEPT_ID, $PROGRAM_ID, $BATCH_ID, $SECTION_ID)
    {
        return $this->db->query("
            select distinct std.STUDENT_ID, e.EXP_ID, e.EXP_NAME, ls.SDL_DT,ls.STDNT_IN_TIME,ls.STDNT_OUT_TIME,ls.IS_ATTEND, std.REGISTRATION_NO,std.FULL_NAME_EN,std.EMAIL_ADRESS,std.MOBILE_NO, s.SESSION_NAME, sy.DINYEAR, d.DEGREE_NAME, dpt.DEPT_NAME, p.PROGRAM_NAME, b.BATCH_TITLE, f.FACULTY_NAME
            FROM STUDENT_PERSONAL_INFO std
            LEFT JOIN INS_SESSION s ON s.SESSION_ID = std.ADM_SESSION_ID
            LEFT JOIN ADM_YSESSION sy ON sy.SESSION_ID = s.SESSION_ID
            LEFT JOIN INS_DEGREE d ON d.DEGREE_ID = std.DEGREE_ID
            ...
            LEFT JOIN ACA_BATCH b ON b.BATCH_ID = std.BATCH_ID
            LEFT JOIN INS_FACULTY f ON f.FACULTY_ID = std.FACULTY_ID
            LEFT JOIN UMS_LAB_SCHEDULE ls ON ls.STUDENT_ID = std.STUDENT_ID
            LEFT JOIN UM_LAB_EXPERIMENT e ON e.EXP_ID = ls.EXP_ID
            where std.ADM_SESSION_ID = '$YSESSION_ID'
            And std.DEGREE_ID='$DEGREE_ID'
            And std.FACULTY_ID='$FACULTY_ID'
            And std.DEPT_ID='$DEPT_ID'
            ...
            And std.SECTION_ID='$SECTION_ID'
            And e.EXP_ID='$EXP_ID'
            And ls.SDL_DT='$SDL_DT'
            GROUP BY std.STUDENT_ID, e.EXP_ID, e.EXP_NAME, ls.SDL_DT,ls.STDNT_IN_TIME,ls.STDNT_OUT_TIME, ls.IS_ATTEND, std.REGISTRATION_NO,std.FULL_NAME_EN,std.EMAIL_ADRESS,std.MOBILE_NO, s.SESSION_NAME, sy.DINYEAR, d.DEGREE_NAME, dpt.DEPT_NAME, p.PROGRAM_NAME, b.BATCH_TITLE, f.FACULTY_NAME

        ")->result_array();
    }

    //findAttenStdData
    public function inOutTimeAttenUpdate($table, $STUDENT_ID, $SDL_DT, $EXP_ID, $where)
    {
        return $this->db->query("
            update $table
            set IS_ATTEND='Y' $where
    ...
    //update
    public function update($table, $attr, $id, $data)
    {
        // get the record that you want to update
        $this->db->select('*');
        $this->db->from($table);
        $this->db->where(array($attr => $id));
        $query = $this->db->get();
        ...
        // updating the record
        $updated_status = $this->db->update($table, $data, array($attr => $id));
    ...
    //Batch Update
    public function update_batch($table, $batch, $attr){
        return $this->db->update_batch($table, $batch, $attr);
    }
    ...
    //This is view so no need join
    function findDistinctDept()
    {
        return $this->db->query("SELECT DISTINCT E.DEPT_NO, E.DEPT_NAME
    ...
    //This is view so no need join
    function findAllEMPfrmView()
    {
        return $this->db->query("SELECT E.EMP_NO, E.NAME||' ('||E.USER_DEFINE_EMP_NO||')' EMP_NAME
            FROM HRV_EMPJOBDETAILS E
            WHERE ACTIVE_STATUS = 'Y'
            ORDER BY E.NAME
        ")->result();
    ...
    //This is view so no need join
    function findAllEMPByIDfrmView($attr)
    {
        return $this->db->query("SELECT E.EMP_NO, E.NAME||' ('||E.USER_DEFINE_EMP_NO||')' EMP_NAME
            FROM HRV_EMPJOBDETAILS E
