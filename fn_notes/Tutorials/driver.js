...
const { bsKey, win10Chrome70, win10IE11 } = require(`./bsCapabilities`);
...
const buildChromeDriver = function (capabilities = webdriver.Capabilities.chrome(), settings = {}) {
  ...
  const chromeOptions = { w3c: false, args: [] };
...
const buildChromeHeadlessDriver = function (capabilities = webdriver.Capabilities.chrome(), settings = {}) {
  require(`chromedriver`); // eslint-disable-line global-require
  const chromeOptions = { w3c: false, args: [`headless`, `disable-gpu`] };
...
const buildDriver = function (platform) {
  ...
  switch (platform.toUpperCase()) {
    ...
    case `CHROME-HEADLESS`:
      driverInstance = buildChromeHeadlessDriver();
      break;
    case `FIREFOX`:
  ...
  return driverInstance.setLoggingPrefs(prefs).build();
};

module.exports = {
  buildAndroidDriver,
  buildChromeDriver,
  buildChromeHeadlessDriver,
  buildBSChromeDriver,
  buildIEDriver,
  buildFirefoxDriver,
  buildDriver,
